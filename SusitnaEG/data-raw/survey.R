...
readxl::read_excel(".\\SusitnaEG\\data-raw\\Susitna run reconstruction data_Jan102018.xlsx",
  ...
  dplyr::mutate(trib = ifelse(grepl("Other WS", trib), "Susitna other", ifelse(grepl("Other ES", trib), "Yentna other", trib)),
                group = ifelse(grepl("Susitna other", trib), "A:H", ifelse(grepl("Yentna other", trib), "J:N", group))) %>%
...
ggplot2::ggplot(survey, ggplot2::aes(x = n)) +
...
sum(is.na(survey_complete))
sum(is.na(survey_expand))
prod(dim(survey_expand[, -1]))

devtools::use_data(survey_complete, pkg = ".\\SusitnaEG", overwrite = TRUE)
devtools::use_data(survey_expand, pkg = ".\\SusitnaEG", overwrite = TRUE)

WriteXLS::WriteXLS(
  c("codes", "survey_complete", "survey_expand"),
  ".\\SusitnaEG\\data-raw\\surveydat_for_steve.xlsx", 
  SheetNames = c("location codes", "complete counts", "expanded counts"),
  na = "NA")
