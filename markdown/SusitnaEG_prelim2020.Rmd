...
Table 2.- Previously released annual escapement estimates for the Susitna River Chinook salmon stock groups obtained by fitting a state-space model to data from 1979 to 2019. These estimates have been superseded by the estimates above.
```{r}
knitr::kable(table_old[["Escapement (CV)"]][40:41, -2], row.names = FALSE, align = "r", escape = FALSE)
```
  
Now that we have a cycle of updating estimates we really need to figure out how to handle it. The estimates changed more that I expected (at least on the Yentna). I'd like more time to try and diagnose why that happened, and if there is a way to stabilize it. My best guess at the moment is just that the model is data poor for the Yentna stock. I do have a few ideas for trying to stabilize these between reports... but I would need more time to test them out. Note, I've had 6 working days to do this and each time I run the model it takes 5 hours.  
  
I also wanted to show you a different way to think about whether we made the goal. As an example consider the Yentna estimate for 2018. Both the new and revised point estimates are within the goal range, although the new estimate is lower (As a reminder... I'm not sure we should go with the new estimate... this is just an example). If we look at a histogram of the posterior probability we see that both estimates are very imprecise relative to the goal range. In both cases there is some chance we were below, within or exceeded the goal. Our confidence does change however, under the old estimate there was ~ 71% chance we were within the goal range while the updated estimate changes that to 50%. 
```{r, message=FALSE, fig.height = 6, fig.width = 10, fig.cap = "Figure 5.- Posterior distribution of 2018 abundance estiamtes."}
goals <- data.frame(name = stock_id, lb = c(9000, 13000, 9000, 13000), ub = c(18000, 25000, 17500, 22000))
...
S_comp_table <- function(post, goal, year_index){
  p <- 
...
Table 3.- Probability that the 2018 escapement for the Susitna River Chinook salmon stock groups was below, within, or above the escapement goal (The first table is for the prior estimates, the second set is for the current estimates).
```{r}
S_comp_table(post_old, goals, 40)
S_comp_table(post_new, goals, 40)
```
  
The situation is worse in 2019 because the point estimate changes from above to within the goal. In that case the probability of being within the goal was ~39% under the old estimate changing to ~58% under the new estiamte.
```{r, message=FALSE, fig.height = 6, fig.width = 10, fig.cap = "Figure 6.- Posterior distribution of 2019 abundance estimates."}
S_comp_plot(post_old, post_new, goals, 41)
```
Table 4.- Probability that the 2019 escapement for the Susitna River Chinook salmon stock groups was below, within, or above the escapement goal (The first table is for the prior estimates, the second set is for the current estimates).
```{r}
S_comp_table(post_old, goals, 41)
S_comp_table(post_new, goals, 41)
```