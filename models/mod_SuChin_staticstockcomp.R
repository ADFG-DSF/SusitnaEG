...
#  
################################################################################
#sensitivity test for 2020 report
#compared to base model this version has: 
#independent alpha
#stock composition does not trend through time
...
model{
  for (s in 1:SG){
	...
	lnalpha.vec[1:(Y - a.min), s] <- lnalpha.y[(A+a.min):(Y+A-1), s]
	  ...
	  lnalpha[s] ~ dnorm(0, 1.0E-6)T(0,)#dnorm(mu.lnalpha, tau.lnalpha)T(0,)  
...
for (t in 1:7) {
    gamma.S2[t] <- Dsum.S2 * pi.S2[t]
    for (y in 1:Y) {
      g.S2[y,t] ~ dgamma(gamma.S2[t],0.1)
      p.S2[y,t] <- g.S2[y,t]/sum(g.S2[y,])
      }
    }
...
for (t in 1:3) {
    gamma.S3[t] <- Dsum.S3 * pi.S3[t]
    for (y in 1:Y) {
      g.S3[y,t] ~ dgamma(gamma.S3[t],0.1)
      p.S3[y,t] <- g.S3[y,t]/sum(g.S3[y,])
      }
    }
...
for (t in 1:5) {
    gamma.S4[t] <- Dsum.S4 * pi.S4[t]
    for (y in 1:Y) {
      g.S4[y,t] ~ dgamma(gamma.S4[t],0.1)
      p.S4[y,t] <- g.S4[y,t]/sum(g.S4[y,])
      }
    }

# MULTINOMIAL COUNTS OF RADIOS TRACKED TO SURVEYED AREAS
for (y in 1:Y) { 
    tele.S2[y, ] ~  dmulti(p.S2[y, ], Ntele.S2[y])  #Eq. 21
