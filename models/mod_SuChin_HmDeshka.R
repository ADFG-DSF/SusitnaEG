...
#Marine harvest 50% Deshka and 50% Yentna (0% east/Talkeetna)
...
for (y in 1:Y) {
  ...
  Hmarine[y] <- mu.Hmarine[y] * sum(N[y, 1] + N[y, 4])  #mu.Hmarine[y] * sum(N[y, ])#NOTE CHANGE
  ...
  IR[y, 1] <- N[y, 1] * (1 - mu.Hmarine[y])  #NOTE CHANGE
  IR[y, 2] <- N[y, 2] 
  IR[y, 3] <- N[y, 3]  
  IR[y, 4] <- N[y, 4] * (1 - mu.Hmarine[y])
  for (s in 1:SG){
