...
wd="H:/My Documents/_Susitna Chinook/analysis_steve/v1.0 hierarchical/"
setwd(wd)
getwd()
...
#alldat=read.csv('SusitnaChinookSSMdata 17Feb18.csv',header=T)
alldat=read.csv('SuChin v1.04 data 17Feb18.csv',header=T)
...
# as.alexander=as.numeric(alldat$as.alexander)  # DELETED FROM CSV FILE
 as.deshka=as.numeric(alldat$as.deshka)
 as.east.su=as.numeric(alldat$as.east.su)
 as.talkeetna=as.numeric(alldat$as.talkeetna)
 as.main.up=as.numeric(alldat$as.main.up)
 as.chulitna=as.numeric(alldat$as.chulitna)
...
par(mfrow=c(1,1))
indices <- cbind(air.surveys,weir.deshka,MR.mainstem,MR.yentna)
...
####Write the model to a .txt file and save in working_dir   ####
model_file_loc=paste(wd, stock,version," mod.txt", sep="")
...
#### Collect posterior stats on scalars and vectors #####################################
      ...
      theta.lake.md          =statsquants[substr(rownames(statsquants),1,8)=="theta[1]",7]
      theta.kahiltna.md      =statsquants[substr(rownames(statsquants),1,8)=="theta[2]",7]
      theta.talachulitna.md  =statsquants[substr(rownames(statsquants),1,8)=="theta[3]",7]
      theta.skwentna.md      =statsquants[substr(rownames(statsquants),1,8)=="theta[4]",7]
      theta.other.yentna.md  =statsquants[substr(rownames(statsquants),1,8)=="theta[5]",7]
...
par(mfrow=c(2,1),mar=c(1,4,4,1)+0.1,ps=12)
...
#  points(pm12345.obs~cyear)
  points(pm1234.obs~cyear)
  points(pm123.obs~cyear)
  points(pm12.obs~cyear)
  points(pm1.obs~cyear)
  lines(pm12345.fit~cyear, lty=2) 
  lines(pm1234.fit~cyear, lty=2) 
  lines(pm123.fit~cyear, lty=2) 
  lines(pm12.fit~cyear, lty=2) 
...
plot(py1234.obs~cyear, type='p', col="black", pch=1, ps=2, xlim=c(1978,2018), ylim=c(0, 1), ylab="Proportion",
  ...
  points(py123.obs~cyear)
  points(py12.obs~cyear)
  points(py1.obs~cyear)
  lines(py1234.fit~cyear, lty=2) 
  lines(py123.fit~cyear, lty=2) 
  lines(py12.fit~cyear, lty=2) 
...
plot(probMSY90~s, ylab="Pr(S > 0.9 MSY)",type="l",lty=1,xlim=c(0,200000),ylim=c(0,1))
lines(probMSY80~s, lty=2)
lines(probMSY70~s, lty=3)
abline(v=c(44000,44000),col="red",lty=1)

# Histogram of Annual Eggs Spawned
hist(S.mn,xlim=c(0,200000),main="",ylab="Freq (Eggs, 10^6)",nclass=12)
abline(v=c(44000,44000),col="red",lty=1)
